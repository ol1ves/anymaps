...
|------|----------------|-------|
| `addMarker` | `id`*, `lat`*, `lng`*, `icon`?, `color`?, `label`?, `title`? | `icon` is an emoji or image URL |
| `updateMarker` | `id`*, then any of `lat`, `lng`, `icon`, `color`, `label`, `title` | only changed fields |
| `removeMarker` | `id`* | |
...
| `flyTo` | `center`* `[lat,lng]`, `zoom`?, `bearing`? | |
...
| `mapClick` | `lat`*, `lng`* | |
| `cameraGranted` | — | lease acquired |
| `cameraDenied` | — | lease held by another widget |
| `cameraRevoked` | `reason`* | `ttl` \| `userGesture` \| `released` |
...
| `record` | object | optional record mapping (section 7.3) |
| `external` | object | external read channels only (section 7.4) |
...
| `time` | JMESPath within each element. Optional. Enables `since`/`until` and `latest`. |
...
Pagination is not supported in the MVP (single request per channel).
...
indexes it per the channel's record mapping and stores the full object.
...
element). The server returns records whole; it never projects fields.
...
- TTL 10 seconds. Any `flyTo` or `fitBounds` from the owner renews it.
- `requestCameraControl` granted if free, else `cameraDenied`.
- Revoke triggers: `releaseCameraControl`, TTL expiry, worker death / disable /
  uninstall, or user map gesture (drag/zoom/scroll). On revoke the owner gets
  `cameraRevoked` with the reason (worker death cannot notify).
